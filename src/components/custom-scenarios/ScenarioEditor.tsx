...
export default function ScenarioEditor({ scenario, onSave, onCancel }: ScenarioEditorProps) {
  ...
  const addItem = (
    type: 'objectives' | 'challenges' | 'tags',
    value: string,
  ...
  const removeItem = (type: 'objectives' | 'challenges' | 'tags', index: number) => {
    setFormData({
      ...formData,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="Первичный приём пациента"
              required
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Установить контакт, собрать анамнез, назначить лечение"
              required
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <Textarea
              ...
              placeholder="Я очень боюсь врачей. У меня негативный опыт..."
              rows={2}
              required
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <Label htmlFor="knowledge">Уровень медицинской грамотности</Label>
              <Select
          ...
          <div>
            <Label htmlFor="initialMessage">Первое сообщение пациента *</Label>
            <Textarea
              id="initialMessage"
              value={formData.initialMessage}
              onChange={(e) => setFormData({ ...formData, initialMessage: e.target.value })}
              placeholder="Здравствуйте, доктор. У меня болит..."
              rows={2}
              required
            />
          </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          <div>
            <Label>Задачи для врача</Label>
            <div className="flex gap-2 mt-2">
              <Input
                value={newObjective}
                onChange={(e) => setNewObjective(e.target.value)}
                placeholder="Проявить эмпатию, собрать анамнез..."
                onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addItem('objectives', newObjective, setNewObjective))}
              />
              <Button type="button" onClick={() => addItem('objectives', newObjective, setNewObjective)}>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {formData.objectives?.map((obj, i) => (
                <Badge key={i} variant="secondary" className="gap-1">
                  {obj}
                  <button type="button" onClick={() => removeItem('objectives', i)}>
                    <Icon name="X" size={12} />
                  </button>
                </Badge>
              ))}
          ...
          <div>
            ...
            <div className="flex gap-2 mt-2">
              <Input
                ...
                placeholder="Страх перед лечением, недоверие..."
                onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addItem('challenges', newChallenge, setNewChallenge))}
              />
              <Button type="button" onClick={() => addItem('challenges', newChallenge, setNewChallenge)}>
                <Icon name="Plus" size={16} />
              </Button>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
              {formData.challenges?.map((ch, i) => (
                <Badge key={i} variant="destructive" className="gap-1">
                  {ch}
                  <button type="button" onClick={() => removeItem('challenges', i)}>
                    <Icon name="X" size={12} />
                  </button>
                </Badge>
              ))}
      ...
      <Card className="p-6">
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="стоматология, страх, первичный приём..."
